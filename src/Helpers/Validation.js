...
function validateUsername(username) {
  ...
  if (username.length > 64) {
    return {
      message: "Username mustn't be longer than 64 characters",
      error: true,
    };
  }
...
function validateNickname(nickname) {
  ...
  if (nickname.length > 64) {
    return {
      message: "Nickname mustn't be longer than 64 characters",
      error: true,
    };
  }
...
function validatePassword(password) {
  ...
  if (password.length > 512) {
    return {
      message: "Password mustn't be longer than 512 characters.",
      error: true,
    };
  }
...
function validateRepeatedPassword(password, repeatedPassword) {
  if (password !== repeatedPassword) {
    return { message: "Passwords don't match.", error: true };
  }
